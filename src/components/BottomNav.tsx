...
export default function BottomNav() {
  ...
  return (
    <nav className="fixed bottom-0 left-0 right-0 z-50 h-16 flex items-center justify-around bg-white border-t-2 border-gray-100 shadow-[0_-4px_16px_rgba(0,0,0,0.08)]" aria-label="주요 메뉴">
      {NAV_ITEMS.map(({ to, key, emoji, color, activeBg, activeBorder }) => {
        ...
        return (
          <Link
            key={to}
            to={to}
            className="flex-1 max-w-36 no-underline"
