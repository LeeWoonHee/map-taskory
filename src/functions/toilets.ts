...
interface ToiletApiRow {
  ...
  OPEN_TM?: string
  CLOSE_TM?: string
  DISABLED_TOILET_YN?: string
  MALE_STOOL_CNT?: string
  FEMALE_STOOL_CNT?: string
...
export const fetchToilets = createServerFn({ method: 'GET' }).handler(
  async (): Promise<{ toilets: PublicToilet[]; source: 'api' | 'static' }> => {
    ...
    try {
      ...
      if (!res.ok) return { toilets: staticToilets, source: 'static' }
      const json = (await res.json()) as Record<string, unknown>
      const service = json?.SearchPublicToiletPOIService as Record<string, unknown> | undefined
      const rows: ToiletApiRow[] = (service?.row as ToiletApiRow[] | undefined) ?? []
